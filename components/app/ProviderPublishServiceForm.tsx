...
export default function ProviderPublishServiceForm({
  ...
  useEffect(() => {
    ...
    setError(null);
    if (service) {
      setJobTypeId(service.jobType?.id ?? "");
      setDescription(service.description ?? "");
      setHourlyRate(service.salaryMin ?? "");
      setRegionId(service.region?.id ?? "");
    } else {
      setJobTypeId("");
      setDescription("");
      setHourlyRate("");
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await fetch(url, {
        ...
        body: JSON.stringify({
          title: selectedJobType?.name || service?.title || "",
          description,
          jobTypeId,
          salaryMin: hourlyRate,
          salaryPeriod: "HEURE", // ✅ spécifique prestataire
          regionId,
    ...
    >
      ...
      >
        ...
        >
          ...
          <div>
            <label
              htmlFor="publish-service-job-type"
          ...
          <div>
            <label
              htmlFor="publish-service-description"
              className={`flex items-center gap-1.5 text-xs uppercase tracking-wide text-gray-500 dark:text-white/50 mb-1.5 ${orbitron.className}`}
            >
            ...
            <textarea
              id="publish-service-description"
              value={description}
            ...
            <div>
              ...
              <div className="relative">
                <input
                  ...
                  disabled={submitting}
                  placeholder="1500"
                  className={`w-full h-10 rounded-lg bg-gray-100 dark:bg-white/5 border border-gray-300 dark:border-white/10 text-sm text-gray-900 dark:text-white/90 px-3 pr-16 focus:outline-none focus:border-[#432dd7]/60 transition-colors disabled:opacity-50 ${orbitron.className}`}
                />
                <span
